<template>
  <section class="populares">
    <h1>{{ $t('recetasPopulares.title') }}</h1>
    <div v-if="filtradosStore.recetasPopulares.length > 0" class="recetas-container">
      <div
        v-for="receta in filtradosStore.recetasPopulares"
        :key="receta.idReceta"
        class="receta-card"
      >
        ...
        <div class="stars">
          <svg
            v-for="n in Math.floor(receta.promedioVotos)"
            :key="`filled-${n}`"
            class="filled"
          ...
          <svg
            v-if="receta.promedioVotos % 1 >= 0.5"
            class="half"
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
          ...
          <svg
            v-for="n in 5 - Math.ceil(receta.promedioVotos)"
            :key="`empty-${n}`"
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
